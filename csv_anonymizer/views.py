...
class StatisticsView(View):  
    def get(self, request):  
        ...
        # Calculer le nombre total de fichiers analysés  
        total_files = len(completed_jobs)  
          
        # Analyser les données pour générer les statistiques d'entités  
        entity_stats = {}  
        total_entities_detected = 0  
          
        # Pour chaque job complété, analyser les données pour les statistiques  
        for job in completed_jobs:  
            job_id = str(job['_id'])  
              
            # Chercher les données CSV associées (si elles existent encore)  
            csv_data_record = collection.find_one({'job_id': job_id})  
              
            if csv_data_record:  
                csv_data = csv_data_record['data']  
                  
                # Analyser les entités dans les données  
                analyzer = AnalyzerEngine()  
    ...
    def calculate_risk_levels(self, entity_stats, total_entities):  
        ...
        high_risk_entities = ['PERSON', 'EMAIL_ADDRESS', 'PHONE_NUMBER', 'CREDIT_CARD', 'IBAN_CODE']  
